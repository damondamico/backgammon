...
class Player:
    ...
    def turn(self, board, dice):
        if self.game_started:
            rc = RuleChecker()
            valid_turns = rc.get_valid_moves(self.color, self.opp_color, dice, board)
            if valid_turns:
                optimalTurn = self.strategy.choose_turn(valid_turns, board, self.color, self.opp_color, dice)
                return optimalTurn
            else:
                return []
        else:
