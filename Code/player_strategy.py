...
class bopHappyStrategy:   
    def __init__(self):
        pass
    def choose_turn(self, turns, board, color, opp_color):
        score = False
        optimalTurn = []
        for turn in turns:
            board_copy = deepcopy(board)
            for move in turn:
                board_copy.make_move(color, opp_color, move[1], move[0])
            new_score = self._score_fun(board_copy, color, opp_color)
            if score == False:
                score = new_score
                optimalTurn = turn
...
class smartStrategy:
    def __init__(self):
        pass
    def choose_turn(self, turns, board, color, opp_color, dice):
        score = False
        optimalTurn = []
